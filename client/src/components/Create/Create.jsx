...
export default function Form() {
    ...
    const handleChange = (event) => {
        ...
        validations(name, errors, setErrors, {...form, [name]: value})
    }
...
const handleSubmit = (event) => {
        ...
        const newErrors = validateForm(form, selectCountries, setErrors);
        const hasErrors = Object.keys(newErrors).length > 0;

        if (hasErrors) {
            window.alert("Revisa que todos los campos hayan sido completados correctamente.");
            setForm(formDefault); // Reiniciar el estado del formulario
            setSelectCountries([]); // Vaciar la lista de países seleccionados
            return;
        }
        ...
        axios.post("http://localhost:3001/activities/", body)
            .then((response) => window.alert("Actividad creada"))
            .catch((error) => console.error("Error al crear la actividad:", error));
